...
export default function EventCarouselBanner() {
  ...
  const onTouchEnd = (e) => {
    ...
    if (Math.abs(diff) > 50) {
      pauseAndNavigate(diff > 0 ? goToNext : goToPrev);
    }
  ...
  useEffect(() => {
    const handler = (e) => {
      if (e.key === "ArrowLeft") pauseAndNavigate(goToPrev);
      if (e.key === "ArrowRight") pauseAndNavigate(goToNext);
      if (e.key === " ") {
        e.preventDefault();
        setIsPaused((p) => !p);
      }
  ...
  return (
    ...
    >
          ...
          >
            ...
            <img
              ...
              className={`absolute inset-0 w-full h-full object-cover transition-transform duration-[10000ms]
                ${active && !isPaused ? "scale-110" : "scale-100"}
              `}
            />
            ...
            <div className="absolute inset-0 flex items-center px-6 md:px-20">
              <div className="max-w-3xl text-white">
                ...
                <div className="flex gap-4">
                  <button
                    onClick={() =>
                      pauseAndNavigate(() => navigate(`/events/${slide._id}`))
                    }
                    className="cursor-pointer px-6 py-3 rounded-xl bg-gradient-to-r from-sky-500 to-purple-600 font-semibold"
                  >
                    Explore Event →
                  </button>

                  <button
                    onClick={() => pauseAndNavigate(() => navigate("/events"))}
                    className="cursor-pointer px-6 py-3 rounded-xl border border-white/30"
                  >
                    View All
                  </button>
                </div>
      ...
      {/* ================= ARROWS ================= */}
      <button
        onClick={() => pauseAndNavigate(goToPrev)}
        className="cursor-pointer absolute left-6 top-1/2 -translate-y-1/2 z-[100] bg-white/10 backdrop-blur p-4 rounded-full"
      >
        ❮
      </button>

      <button
        onClick={() => pauseAndNavigate(goToNext)}
        className="cursor-pointer absolute right-6 top-1/2 -translate-y-1/2 z-[100] bg-white/10 backdrop-blur p-4 rounded-full"
      >
        ❯
      </button>

      {/* ================= BOTTOM CONTROLS ================= */}
      <div className="absolute bottom-6 w-full z-[100] flex flex-col items-center gap-4">
        {/* Thumbnails */}
        <div className="flex gap-2">
          {events.map((e, i) => (
            <button
              key={i}
              onClick={() => pauseAndNavigate(() => goToSlide(i))}
              className={`cursor-pointer w-14 h-10 overflow-hidden rounded-lg ${
                current === i ? "ring-2 ring-white" : "opacity-60"
              }`}
            >
              <img
                src={e.image}
        ...
        <div className="w-64 h-1 bg-white/20 rounded-full overflow-hidden">
          <div
            className="h-full bg-gradient-to-r from-sky-400 to-purple-500 transition-all duration-100"
            style={{ width: `${progress}%` }}
          />
        </div>
        ...
        <button
          ...
          className="cursor-pointer text-white/70 text-sm"
        >
          {isPaused ? "▶ Play" : "❚❚ Pause"}
        </button>
      </div>
